...
export function Chatface() {
  ...
  const handleKeyDown = (event) => {
    if (event.key === 'Enter' && !event.shiftKey) { // 檢查是否按下Enter並且沒有按下Shift
  ...
  return (
    <>
      ...
      <Card className="mx-3 -mt-16 mb-6 lg:mx-4 border border-blue-gray-100 ">
        ...
        <div className="h-[calc(100vh-260px)] sm:max-h-screen-50 md:max-h-screen-75 lg:max-h-screen-90  overflow-y-auto">
  {messages.length === 0 ? (
    <div className="flex justify-center items-center h-full">
      <p className="text-sm text-gray-500">現在可以開始對話了</p>
    </div>
  ) : (
    messages.map((message, index) => (
      <div key={index} className="flex items-start gap-2.5 mb-2">
        ...
        <div className="flex flex-col w-full max-w-[320px] p-4 bg-gray-100 rounded-xl">
          ...
          <div class="flex w-full flex-row items-center gap-2 rounded-[99px] border border-gray-900/10 bg-gray-900/99 p-2">
            <div class="flex">
              <button
